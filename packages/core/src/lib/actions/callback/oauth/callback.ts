...
export async function handleOAuth(
  ...
  // Falls back to auth.khulnasoft.com if the user only passed params
  if (
    (!token?.url || token.url.host === "auth.khulnasoft.com") &&
    (!userinfo?.url || userinfo.url.host === "auth.khulnasoft.com")
  ) {
    ...
    as = {
      issuer: provider.issuer ?? "https://auth.khulnasoft.com", // TODO: review fallback issuer
