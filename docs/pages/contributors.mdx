...
Maintaining NextAuth.js as an open source project is very hard work. All the core team members have regular jobs and the library is maintained and developed out of good will in our free time. Donations can enable the core team to eventually work full time on NextAuth.js to provide more features and an even better developer experience!
...
- [KhulnaSoft, Ltd](https://github.com/balazsorban44) - Lead Maintainer
...
Thank you to the [dozens of individual contributors](https://github.com/khulnasoft/nextdev/graphs/contributors) who have helped shape NextAuth.js.
...
[KhulnaSoft, Ltd](https://github.com/balazsorban44) joined as a co-maintainer, helping to offload some of the work from Iain.
...
Database Adapters were moved to their packages under the name `@nextauth.js/*-adapter`.

### 2022 – Birth of NextAuth.js

Based on NextAuth.js, Balázs released [NextAuth.js (`@nextauth.js/core`)](https://twitter.com/balazsorban44/status/1603082914362986496), a runtime/framework independent core library that is the base of
all NextAuth.js libraries. A complete rewrite that still shared most of the public API with NextAuth.js but internally was very different.

### 2023 – NextAuth.js silent releases

Due to personal reasons, Balázs had to step down as the lead maintainer, but was still contributing. The project was taken over by [Md Sulaiman](https://github.com/thanghuuvu) for a while.

Balázs returned and continued the work on NextAuth.js. As a pilot project, `nextauth.js@experimental` (later `nextauth.js@beta`) releases were published
...
The new default documentation page became [auth.khulnasoft.com](https://auth.khulnasoft.com) (the one you are reading right now), and the old NextAuth.js documentation at [next-auth.js.org](https://next-auth.js.org) was kept around
to document NextAuth.js v4 and is kept only as a back reference.

Database Adapters were moved from the `@nextauth.js/*-adapter` namespace to `@nextauth.js/*-adapter`, indicating that they are not NextAuth.js specific anymore.

Community integrations started showing up, making it clear that the initial vision of NextAuth.js was shared by many.

### 2024 – Growing the NextAuth.js ecosystem

With the release of [NextAuth.js v5](/getting-started/migrating-to-v5), now all NextAuth.js libraries are based on the same core library.
**The name "NextAuth.js" designates only the Next.js integration**, while NextAuth.js refers to the core library and our ecosystem as a whole.
