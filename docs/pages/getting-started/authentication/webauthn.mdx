...
- `nextauth.js@5.0.0-beta.8` or above
- `@nextauth.js/prisma-adapter@1.3.0` or above
...
The `@simplewebauthn/browser` peer dependency **is only required for custom signin pages**. If you're using the NextAuth.js default pages, you can skip installing that peer dependency.

### Apply the required schema Migrations

This is the raw SQL migration for PostgreSQL, for more details including example migrations for other databases, check out the updated Prisma schemas at the [`@nextauth.js/prisma-adapter` docs](/getting-started/adapters/prisma).
...
If you're using the built-in NextAuth.js pages, then you are good to go! Navigating to your `/signin` route should include a "Signin with Passkeys" button.
