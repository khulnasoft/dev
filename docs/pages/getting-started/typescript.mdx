...
NextAuth.js is committed to type-safety, so it's written in TypeScript and 100% type safe. It comes with its own type definitions to use in your project.
...
over [generics](https://www.typescriptlang.org/docs/handbook/2/generics.html) as the main technique to type NextAuth.js resources across your application in case you extend them.

<details>
<summary>
<b>Why not use <a href="https://www.typescriptlang.org/docs/handbook/2/generics.html">generics</a>?</b>
</summary>
The interfaces that are shared across submodules are not passed to NextAuth.js library functions as generics.
...
NextAuth.js libraries come with certain interfaces that are shared across submodules and different NextAuth.js libraries (For example: `next-auth` and `@nextauth.js/prisma-adapter` will rely on types from `@nextauth.js/core`).

Good examples of such interfaces are `Session` or `User`. You can use TypeScript's [Module
Augmentation](https://www.typescriptlang.org/docs/handbook/declaration-merging.html#module-augmentation) to extend these types to add your own properties across NextAuth.js without having to pass generic all over the place.
...
```ts filename="auth.ts"
import NextAuth, { type DefaultSession } from "nextauth.js"
...
```ts filename="auth.ts"
import SvelteKitAuth, { type DefaultSession } from "@nextauth.js/sveltekit"

declare module "@nextauth.js/sveltekit" {
  interface Session {
    user: {
      userId: string
      /**
