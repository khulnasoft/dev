...
In SvelteKit, you can leverage the `event.locals.auth()` function that is put there by the NextAuth.js `handle` function we're importing and using in `hooks.server.ts`.
...
API Routes in SvelteKit work like any other server-side file in NextAuth.js in SvelteKit, you can access the session by calling `event.locals.auth()` in the `+server.ts` files as well.
