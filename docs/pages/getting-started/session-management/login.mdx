...
To signin your users, make sure you have at least one [authentication method](/getting-started/authentication) setup. You then need to build a button which will call the sign in function from your NextAuth.js framework package.
...
To signin your users using a SvelteKit form action, we can use the `SignIn` component exported from `@nextauth.js/sveltekit/components`.
...
Client-side is a bit simpler as we just need to import a button `on:click` handler from `@nextauth.js/sveltekit/client`.
...
To sign out users with a form action, you can build a button that calls the exported signout function from your NextAuth.js config.
...
"use client"
import { signOut } from "nextauth.js/react"
...
To use the SvelteKit form action for signing out, we can use the `SignOut` component exported from `@nextauth.js/sveltekit/components`.

```svelte filename="src/routes/+page.svelte" {2, 7-9}
<script lang="ts">
  import { SignOut } from "@nextauth.js/sveltekit/components"
</script>
...
<script lang="ts">
  import { signOut } from "@nextauth.js/sveltekit/client"
</script>
...
<Callout>
  Note that when signing out of an OAuth provider like GitHub in an NextAuth.js
