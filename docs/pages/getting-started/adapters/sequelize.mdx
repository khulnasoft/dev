...
```bash npm2yarn
npm install @nextauth.js/sequelize-adapter sequelize
```
...
import SequelizeAdapter from "@nextauth.js/sequelize-adapter"
...
```ts filename="./auth.ts"
import NextAuth from "nextauth.js"
import SequelizeAdapter, { models } from "@nextauth.js/sequelize-adapter"
import Sequelize, { DataTypes } from "sequelize"

const sequelize = new Sequelize("sqlite::memory:")

export const { handlers, auth, signIn, signOut } = NextAuth({
  // https://auth.khulnasoft.com/reference/providers/
  providers: [],
  adapter: SequelizeAdapter(sequelize, {
    models: {
