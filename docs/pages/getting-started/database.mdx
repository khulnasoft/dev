...
<Callout type="info">
  NextAuth.js integrations save sessions in a cookie by default. Therefore, setting
...
**Database Adapters** are the bridge we use to connect NextAuth.js to your database. For instance, [when implementing magic links](/getting-started/authentication/email), the Email provider will require you to setup a database adapter to be able to store the [verification tokens](/concepts/database-models#verificationtoken) present on the links.

## Official Adapters

Below is a list of official adapters that are distributed as their own packages under the `@nextauth.js/`
namespace. Their source code is available in the [`khulnasoft/nextdev`
monorepo](https://github.com/khulnasoft/nextdev/tree/main/packages). If you're going to create a database adapter, please make sure you familiarise yourself [with the models](/concepts/database-models) NextAuth.js expects to be present and check out our "[creating a database adapter](/guides/creating-a-database-adapter)" guide.
