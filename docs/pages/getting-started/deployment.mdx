...
<Callout>
  For consistency, we recommend prefixing all NextAuth.js environment variables with
...
NextAuth.js libraries require you to set an `AUTH_SECRET` environment variable. This is used to encrypt cookies and tokens. It should be a cryptographically secure random string of at least 32 characters:
...
<Callout type="info">
NextAuth.js supports **environment variable inference**, meaning that if you name your provider environment variables following a specific syntax, you won't need to explicitly pass them to the providers in your configuration.
...
to `true`. This tells NextAuth.js to trust the `X-Forwarded-Host` header from the reverse proxy. NextAuth.js will automatically infer this to be true if we detect the environment variable indicating that your application is running on one of the supported hosting providers. Currently `VERCEL` and `CF_PAGES` (Cloudflare Pages) are supported.
...
> **_NOTE:_** Some providers (eg Apple) do not support [redirect proxy](https://github.com/khulnasoft/nextdev/blob/3ec06842682a31e53fceabca701a362abda1e7dd/packages/core/src/lib/utils/providers.ts#L48) usage.

This environment variable is designed for advanced use-cases only, when using NextAuth.js as a proxy for preview deploys, for example. For more details, see the [securing preview deploys](#securing-a-preview-deployment) section below.
...
2. When using an OAuth provider, make sure the callback URL for your production URL is setup correctly. Many OAuth providers will only allow you to set 1 `callbackUrl` per OAuth application. In which case, you'll need to create separate applications for each environment (development, production, etc.). Other providers, like Google, allow you to add many `callbackUrl`s to one application.
   ...
   - All other frameworks (`@nextauth.js/sveltekit`, `@nextauth.js/express`, etc.), by default, will use the path `/auth/callback/[provider]`.
3. Deploy! After having setup those two prerequisites, you should be able to deploy and run your NextAuth.js application on Netlify, Vercel, etc.
...
To pass on your current user's details on to your observability tools, you can use the callbacks provided by NextAuth.js. For example, in the `session` callback, you could pass the `user.id` on to Sentry.
...
NextAuth.js can also be deployed anywhere you can deploy your framework of choice. Check out the framework's documentation on self-hosting.

### Docker

In a Docker environment, make sure to set either `trustHost: true` in your NextAuth.js configuration or the `AUTH_TRUST_HOST` environment variable to `true`.

Our example application is also hosted via Docker [here](https://nextjs-docker-example.auth.khulnasoft.com) (see the [source code](https://github.com/khulnasoft/nextdev-example)). Below is an example `Dockerfile` for a Next.js application using NextAuth.js.
...
2. In **both the preview and stable environment**, set `AUTH_REDIRECT_PROXY_URL` to that stable deployment URL, including the path from where NextAuth.js handles the routes. Eg.: (`https://auth.yourdomain.com/api/auth`). If the variable is not set in the stable environment, the proxy functionality will not be enabled!
...
To support preview deployments, NextAuth.js requires a deployment URL that is stable across deploys as a redirect proxy server.
