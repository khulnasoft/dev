...
Our example application is also hosted via Docker [here](https://nextjs-docker-example.auth.khulnasoft.com) (see the [source code](https://github.com/khulnasoft/nextdev-example)). Below is an example `Dockerfile` for a Next.js application using Auth.js.
