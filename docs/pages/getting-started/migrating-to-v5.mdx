...
- NextAuth.js now builds on `@nextauth.js/core` with stricter [OAuth](https://www.ietf.org/rfc/rfc6749.html)/[OIDC](https://openid.net/specs/openid-connect-core-1_0.html) spec-compliance, which might break some existing OAuth providers. See our [open issues](https://github.com/khulnasoft/nextdev/issues?q=is%3Aopen+is%3Aissue+label%3Aproviders) for more details.
...
The configuration file should look very similar to your previous route-based NextAuth.js configuration. Except that we're doing it in a new file in the root of the repository now, and we're exporting methods to be used elsewhere. Below is a simple example of a v5 configuration file.
...
2. There is no need to install `@nextauth.js/core` to import the provider definitions from, these come from `next-auth` itself.
...
NextAuth.js has had a few different ways to authenticate server-side in the past, so we've tried to simplify this as much as possible.
...
Beginning with `next-auth` v5, you should now install database adapters from the `@nextauth.js/*-adapter` scope, instead of `@nextauth.js/*-adapter`. Database adapters don't rely on any Next.js features, so it made more sense to move them to this new scope.

```diff
- npm install @nextauth.js/prisma-adapter
+ npm install @nextauth.js/prisma-adapter
```
...
NextAuth.js supports two [session strategies](/concepts/session-strategies). When you are using an adapter, it will default to the `database` strategy. **Unless your database and its adapter is compatible with the Edge runtime/infrastructure, you will not be able to use the `"database"` session strategy**.
...
1. Create an `auth.config.ts` file which exports an object containing your NextAuth.js configuration options. You can put all common configuration here **which does not rely on the adapter**. Notice this is exporting a configuration object only, we're not calling `NextAuth()` here.
...
3. In your middleware file, import the configuration object from your first `auth.config.ts` file and use it to lazily initialize NextAuth.js there. In effect, initialize NextAuth.js separately with all of your common options, but **without the edge incompatible adapter**.
...
For more information in general about edge compatibility and how NextAuth.js fits into this, check out our [edge compatibility article](/guides/edge-compatibility).
...
- The `AUTH_TRUST_HOST` environment variable serves the same purpose as setting `trustHost: true` in your NextAuth.js configuration. This is necessary when running NextAuth.js behind a proxy. When set to true we will trust the `X-Forwarded-Host` and `X-Forwarded-Proto` headers passed to the app by the proxy to auto-detect the host URL (`AUTH_URL`)
...
- The following types are now exported from all framework packages like `next-auth` and `@nextauth.js/sveltekit`:
...
- All `Adapter` types are re-exported from `/adapters` in the framework packages as well, i.e. from `next-auth/adapters`, `@nextauth.js/sveltekit/adapters`, etc.
...
We hope this migration goes smoothly for everyone! If you have any questions or get stuck anywhere, feel free to create [a new issue](https://github.com/khulnasoft/nextdev/issues/new) on GitHub, or come chat with us in the [Discord](https://discord.auth.khulnasoft.com) server.
