...
NextAuth.js libraries can be configured to use this `refresh_token` to obtain a new `access_token` without requiring the user to sign in again.
...
<Callout type="info">
  ...
  at the same time. The NextAuth.js team is aware of this and would like to provide
...
<Callout>
  ...
  secure. To mitigate risks with the `strategy: "jwt"`, NextAuth.js libraries store
...
```ts filename="./auth.ts"
import NextAuth, { type User } from "nextauth.js"
...
import { useEffect } from "react"
import { signIn, useSession } from "nextauth.js/react"
