...
NextAuth.js adapters allow you to integrate with any (even multiple) database/back-end service, even if we don't have an [official package](https://github.com/khulnasoft/nextdev/tree/main/packages) available yet. (We welcome PRs for new adapters! See the [guidelines](#official-adapter-guidelines) below.)

NextAuth.js adapters are very flexible, and you can implement only the methods you need, and only create the database tables/columns that are actually going to be used.

An NextAuth.js adapter is a function that receives an ORM/database client and returns an object with methods (based on the [`Adapter` interface](/reference/core/adapters#adapter)) that interact with the database. The same database Adapter will be compatible with any NextAuth.js library.

Optionally, you can run our [Adapter tests](https://github.com/khulnasoft/nextdev/blob/main/packages/utils/adapter.ts) on your adapter to ensure it is compliant with the NextAuth.js.

## User management

NextAuth.js differentiates between users and accounts. A user can have multiple accounts. An account is created for each provider type the user signs in with for the first time. For example, if a user signs in with Google and then with Facebook, they will have two accounts, one for each provider. The first provider the user signs in with will also be used to create the user object. See the [`profile()` provider method](/reference/core/providers#profile).
...
<div style={{display: "grid", gridTemplateColumns: "1fr 1fr"}}>
  <span>
    ...
    Not yet invoked by NextAuth.js:
...
<Callout type="info">
  Although NextAuth.js doesn't require it, for basic display purposes, we recommend
...
<Callout type="info">
  Although NextAuth.js doesn't require it, the `Account` table typically saves
...
NextAuth.js can manage sessions in two ways. Learn about them and their advantages and disadvantages at [Concepts: Session strategies](/concepts/session-strategies).
...
When you want to support email/passwordless login, NextAuth.js uses a database to store temporary verification tokens that are tied to a user's email address.
...
<Callout type="info">
  ...
  [repository](https://github.com/khulnasoft/nextdev).
</Callout>

If you created an adapter and want us to distribute it as an official package, please make sure it meets the following requirements. Check out this [existing adapter](https://github.com/khulnasoft/nextdev/tree/main/packages/adapter-prisma) to learn about the package structure, required files, test setup, config, etc.

1. The Adapter _must_ implement all methods of the [`Adapter` interface](/reference/core/adapters#adapter)
1. [Adapter tests](https://github.com/khulnasoft/nextdev/blob/main/packages/utils/adapter.ts) _must_ be included and _must_ pass. Docker is favored over services, to make CI resilient to network errors and to reduce the number of GitHub Action Secrets (which also lets us run these tests in fork PRs)
...
1. Configure the monorepo to help us maintain the package
   ...
   - Add a (preferably `.svg`) logo to [this directory](https://github.com/khulnasoft/nextdev/tree/main/docs/public/img/adapters)
   - Add the Adapter to our GitHub workflow files [here](https://github.com/khulnasoft/nextdev/tree/main/.github/workflows/release.yml#L12) and [here](https://github.com/khulnasoft/nextdev/tree/main/.github/pr-labeler.yml)
   - Make sure to [`.gitignore` generated files](https://github.com/khulnasoft/nextdev/tree/main/.gitignore#L58) if there are any

1. The Adapter _must_ be able to handle any property coming from the user

   ORMs/database clients might have their own data types, but NextAuth.js expects these to be normalized as plain JavaScript objects for consistency. If your ORM/database client does not convert automatically, you need to convert the values when reading/writing from/to the database.
...
You can take advantage of the types that comes with the framework packages (i.e. `next-auth/adapters`, `@nextauth.js/sveltekit/adapters`).
...
- [Official adapters' source code](https://github.com/khulnasoft/nextdev/tree/main/packages)
