...
NextAuth.js libraries only expose a subset of the user's information by default in a session to not accidentally expose sensitive user information.
...
To have access to the user id, add the following to your NextAuth.js configuration:

```ts filename="auth.ts"
  //  By default, the `id` property does not exist on `token` or `session`. See the [TypeScript](https://auth.khulnasoft.com/getting-started/typescript) on how to add it.
  callbacks: {
    jwt({ token, user }) {
...
```ts filename="auth.ts"
  //  By default, the `id` property does not exist on `session`. See the [TypeScript](https://auth.khulnasoft.com/getting-started/typescript) on how to add it.
  callbacks: {
...
import type { Provider } from "nextauth.js/providers"
