...
NextAuth.js can be configured to display a custom error page when something goes wrong during the user authentication flow (sign in, sign out, etc..).

In order to override NextAuth.js's `/api/auth/error` page we have to define our custom page in the AuthConfig:
...
Using the [example app](https://github.com/khulnasoft/nextdev-example), let's build a simple custom error page by creating `app/error/page.tsx`
...
NextAuth.js forwards the following errors as error query parameters in the URL to our custom error page:
...
Now, when an error happens, NextAuth.js will redirect the user to our custom error page:
