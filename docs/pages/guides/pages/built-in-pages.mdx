...
NextAuth.js comes by default with a set of pages that are presented to the user as they go through their authentication journey (sign up, sign in, sign out, error, etc...). This is helpful so that you don't need to write those from scratch when using the library first time. The UI created is based on the providers specified in your configuration file.
...
If we added the [Google](/getting-started/providers/google) provider to our NextAuth.js config file (`./auth.ts`), then a 3rd option to sign in with Google would be available.
