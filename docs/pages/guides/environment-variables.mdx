...
NextAuth.js is automatically configured to pick the right environment variables for `clientId` and `clientSecret` when using an [official OAuth provider](/getting-started/authentication/oauth).
...
Then in your NextAuth.js configuration file, the `provider` array is simplified to this.
...
import Google from "@nextauth.js/qwik/providers/google"
import Twitter from "@nextauth.js/qwik/providers/twitter"
