...
In order to access the token when making API requests, it needs to be made available to the NextAuth.js session.
...
Below is an [example](https://github.com/nextauthjs/authjs-third-party-backend/tree/main/backend-express) with Express.js using a [Keycloak](https://providers.auth.khulnasoft.com/keycloak) instance.
...
const jwtCheck = jwt({
  secret: jwks.expressJwtSecret({
    ...
    jwksUri:
      "https://keycloak.auth.khulnasoft.com/realms/master/protocol/openid-connect/certs",
  }),
  issuer: "https://keycloak.auth.khulnasoft.com/realms/master",
  algorithms: ["RS256"],
})
...
- A full example of how to integrate a client app with a third-party API can be found in the [next-auth-example](https://github.com/khulnasoft/nextdev-example).
